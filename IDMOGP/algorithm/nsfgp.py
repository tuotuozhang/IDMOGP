...
# logging results
logging.basicConfig(level=logging.INFO, filename=file_name + 'nsfgp_main.log')
logging.info('#############Start##############')
logging.info('Algorithm name: NSFGP.py ')
...
def GPMain(randomSeeds):
    ...
    pop, log, best_front, hof = evalGP.nsgpSimple(pop, toolbox, cxProb, mutProb, elitismProb, generation, pset,
...
if __name__ == "__main__":
    ...
    for i, ind in enumerate(best_front):
        ...
        logging.info(ind)
        logging.info('test results-' + f'individual{i}: ' + str(testResults))
