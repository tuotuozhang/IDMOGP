...
# logging results
logging.basicConfig(level=logging.INFO, filename=file_name + 'nsfgp_exp_main.log')
logging.info('#############Start##############')
logging.info('Algorithm name: NSFGP_exp.py ')
...
def evalTrain(individual):
    ...
    nf_score = 100 * numpy.exp(-numpy.sqrt(nf))
    nf_score = 100 * numpy.exp(-nf/100)
...
def GPMain(randomSeeds):
    ...
    pop, log, best_front, hof, pop_store = evalGP.nsgpSimple(pop, toolbox, cxProb, mutProb, elitismProb, generation, pset,
...
if __name__ == "__main__":
    ...
    # save_path = '../results/nsfgp_exp/'
    save_path = '/vol/grid-solar/sgeusers/yingbi/MOGP_TUO/results/nsfgp_exp/'
    ...
    train_results = []
    test_acc = numpy.zeros((len(pop), 1))
    test_nf = numpy.zeros((len(pop), 1))
    test_nf_score = numpy.zeros((len(pop), 1))
    for i, ind in enumerate(pop):
        ...
        # nf_score = 100 * numpy.exp(-numpy.sqrt(num_features))
    ...
    numpy.save(save_path+f'{randomSeeds}_{dataSetName}_train_results.npy', train_results)
    numpy.save(save_path + f'{randomSeeds}_{dataSetName}_test_results.npy', test_results)
