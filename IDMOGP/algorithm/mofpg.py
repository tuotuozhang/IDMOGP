...
'based on fgp16, change the fitness function'
#
...
# data_path = '/nfs/scratch/biyi/datasets/ssmultiTask/crossD/'
...
x_train, y_train, x_test, y_test = load_data(dataSetName, data_path)
# x_train, y_train, x_test, y_test = load_data(dataSetName, path=data_path)
# print(x_train.shape)
# _, n, bound1, bound2 = x_train.shape
...
logging.basicConfig(level=logging.INFO, filename=str(randomSeeds) + '_' + dataSetName + '_FGP2222.log')
logging2 = logging.getLogger()
logging2.info('#############Strat##############')
logging2.info('Algorithm name: FGP.py ')
##GP
##GP
...
def evalTrain(individual):
    ...
    try:
        ...
        kf = StratifiedKFold(n_splits=n_cv, random_state=5)
        acc = 0
