...
indicator = str(sys.argv[3])

# randomSeeds = 1
# dataSetName = 'att'
# indicator = ['tchebycheff', 'projection']
# num_instance = 3
...
x_train, y_train, x_test, y_test = deal_data(x_train, y_train, x_test, y_test, dataSetName, num_instance, n_class)
file_name = str(randomSeeds) + '_' + indicator + '_' + str(dataSetName) + '_'
#################################################################################################################################
# parameters:
population = 100
generation = 50
...
maxDepth = 10
# logging results
logging.basicConfig(level=logging.INFO, filename=file_name + 'guiding_mofgp_main.log')
logging.info('#############Start##############')
...
def GPMain(randomSeeds):
    ...
    pop, log, best_front, hof, pop_store = evalGP.Guiding_MOFGP(pop, toolbox, cxProb, mutProb, elitismProb, generation, pset,
                                                     indicator=indicator, stats=mstats, verbose=True)
...
if __name__ == "__main__":
    ...
    save_path = '../results/guiding_mofgp/'
    ...
    for i, ind in enumerate(best_front):
        ...
        print(ind)
        print(testResults)
        logging.info('individual{i}: ')
        logging.info(ind)
        logging.info('test results-' + 'individual{i}: ' + str(testResults))
        print(train_tf.shape, test_tf.shape)
        num_features = train_tf.shape[1]
        logging.info('Num Features-' + 'individual{i}: ' + str(num_features))
        logging.info('test time-' + 'individual{i}: ' + str(testTime))
        endTime = time.process_time()
