...
from deal_with_data2 import deal_data
...
x_train_select, x_test, y_train_select, y_test = deal_data(x_train, y_train, x_test, y_test, dataSetName, num_instance, n_class)
x_test = x_test[:100]
y_test = y_test[:100]
print(x_train_select.shape, x_test.shape, n_class)
#################################################################################################################################
#parameters:
population=100
generation=50
...
initialMaxDepth=6
maxDepth=8
n_cv = min(num_instance, 10)
nf_size = x_train.shape[1] * x_train.shape[2]
#logging results
logging.basicConfig(level=logging.INFO, filename=file_name+'_mofGP_main.log')
logging.info('#############Strat##############')
logging.info('Algorithm name: MOFGP.py ')
logging.info('Training set shape: '+str(x_train_select.shape))
...
def evalTrainb(individual):
    ...
    try:
        ...
        acc = 0
        for train_index, test_index in kf.split(y_train_select, y_train_select):
            x_train_eval, x_test_eval = x_train_select[train_index], x_train_select[test_index]
            y_train_eval, y_test_eval = y_train_select[train_index], y_train_select[test_index]
...
def GPMain(randomSeeds):
    ...
    pop, log, best_ind_over, hof = evalGP.moeaSimple(pop, toolbox, cxProb, mutProb, elitismProb, generation, pset,
...
if __name__ == "__main__":
    ...
    train_tf, test_tf, trainLabel, testL, testResults = evalTest(toolbox, hof[0], x_train_select, y_train_select, x_test, y_test)
    saveFile.saveLog(file_name+'all_pop.pickle', pop)
    saveFile.saveLog(file_name+'best_pop.pickle', hof)
    saveFile.saveLog(file_name+'best_pop_gen.pickle', best_ind_over)
    ...
    saveFile.saveAllResults(file_name, dataSetName, hof, log,
